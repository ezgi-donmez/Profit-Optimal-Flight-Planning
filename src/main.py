...
main.py
----------------
Complete experiment runner for the Profit-Optimal Flight Planning MILP.
DS502 Path B
...
─────────
  ...
  9. Large baseline       16 routes × 12 months × 6 aircraft  ← max within license
...
───────────────
  ...
  Full dataset (30 × 12 × 6) requires ~2900 constraints and that exceeds limit.
  16 routes × 12 months × 6 aircraft stays under ~1960 constraints.
...
def print_results(res, title=""):
    ...
    print("=" * 72)
    print(f"  {title}")
    print(f"  delta={res['delta']:.2f} | alpha={res['alpha']:.2f} | "
    ...
    for (r, t) in served:
        flights = {(rr, tt, aa): v for (rr, tt, aa), v
                   in res['x_vals'].items() if rr == r and tt == t}
    ...
    print()
    print(f"  Aircraft deployments (u=1):")
...
def print_table(results, label=""):
    ...
    print("=" * 100)
    print(f"  {'delta':>5}  {'alpha':>5}  {'Nmin':>4}  {'Nm':>3}  "
          f"{'gamma':>5}  {'neg':>4}  {'Status':>10}  "
          f"{'Objective (USD)':>22}  {'Rts':>4}  {'Open':>4}  "
          f"{'u=1':>4}  {'Time':>7}")
    print("  " + "-" * 98)
    for r in results:
        obj  = (f"${r['obj_value']:>20,.2f}" if r['obj_value'] is not None
                else "                   N/A")
        nrts = len(r['y_vals'])
        nopn = len(r['z_vals'])
        ndep = len(r['u_vals'])
        print(f"  {r['delta']:>5.2f}  {r['alpha']:>5.2f}  "
...
if __name__ == "__main__":
    ...
    res_s = build_and_solve(small, delta=0.0, alpha=1.0,
                            N_min=2, N_maint=2, gamma_maint=0.20,
    ...
    for d in [0.0, 0.10, 0.25]:
        r = build_and_solve(small, delta=d, alpha=1.0,
                            N_min=2, N_maint=2, gamma_maint=0.20,
                            time_limit=120, verbose=False)
        fuel.append(r)
        tag = f"${r['obj_value']:,.2f}" if r['obj_value'] else r['status']
    ...
    for a in [0.30, 0.60, 1.00]:
        r = build_and_solve(small, delta=0.0, alpha=a,
                            N_min=2, N_maint=2, gamma_maint=0.20,
                            time_limit=120, verbose=False)
        cap.append(r)
        tag = f"${r['obj_value']:,.2f}" if r['obj_value'] else r['status']
    ...
    nmin = []
    for n in [1, 2, 3]:
        r = build_and_solve(small, delta=0.0, alpha=1.0,
                            N_min=n, N_maint=2, gamma_maint=0.20,
                            time_limit=120, verbose=False)
        nmin.append(r)
        tag = f"${r['obj_value']:,.2f}" if r['obj_value'] else r['status']
    ...
    for nm in [1, 2, 3]:
        r = build_and_solve(small, delta=0.0, alpha=1.0,
                            N_min=2, N_maint=nm, gamma_maint=0.20,
                            time_limit=120, verbose=False)
        nmnt.append(r)
        tag = f"${r['obj_value']:,.2f}" if r['obj_value'] else r['status']
    ...
    for g in [0.0, 0.10, 0.20, 0.30]:
        r = build_and_solve(small, delta=0.0, alpha=1.0,
                            N_min=2, N_maint=2, gamma_maint=g,
                            time_limit=120, verbose=False)
        gam.append(r)
        tag = f"${r['obj_value']:,.2f}" if r['obj_value'] else r['status']
    ...
    for fc in [0, 50_000, 100_000]:
        ...
        r = build_and_solve(small_fc, delta=0.0, alpha=1.0,
                            N_min=2, N_maint=2, gamma_maint=0.20,
                            time_limit=120, verbose=False)
        act.append(r)
        tag = f"${r['obj_value']:,.2f}" if r['obj_value'] else r['status']
    ...
    res_m = build_and_solve(medium, delta=0.0, alpha=1.0,
                            N_min=2, N_maint=2, gamma_maint=0.20,
    ...
    print("# Gurobi license (~1960 constraints < 2000 limit).")
    print("# For the full 30-route dataset: obtain a free Gurobi academic")
    print("# license at gurobi.com/academia and change n_routes=16 to 30.")
    print("#" * 72)

    large = load_data(n_routes=16, n_months=12, n_aircraft=6, K_ct_value=1)
    print_data_summary(large)

    res_l = build_and_solve(large, delta=0.0, alpha=1.0,
                            N_min=2, N_maint=2, gamma_maint=0.20,
                            time_limit=300, verbose=True)
    ...
    lfuel = []
    for d in [0.0, 0.10, 0.25]:
        r = build_and_solve(large, delta=d, alpha=1.0,
                            N_min=2, N_maint=2, gamma_maint=0.20,
    ...
    for g in [0.0, 0.20, 0.40]:
        r = build_and_solve(large, delta=0.0, alpha=1.0,
                            N_min=2, N_maint=2, gamma_maint=g,
                            time_limit=300, verbose=False)
        lgam.append(r)
        tag = f"${r['obj_value']:,.2f}" if r['obj_value'] else r['status']
